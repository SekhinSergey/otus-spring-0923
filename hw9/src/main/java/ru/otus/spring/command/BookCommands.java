...
public class BookCommands {
    ...
    @SuppressWarnings("all")
    @ShellMethod(value = "Find all books by ids", key = "abbyids")
    public String findAllBooksByIds(Set<Long> ids) {
        List<Book> books = bookService.findAllByIds(ids);
        if (books.isEmpty()) {
            return "No books with ids %d found".formatted(ids);
        }
        return books.stream()
                .map(bookConverter::bookToString)
                .collect(Collectors.joining("," + System.lineSeparator()));
